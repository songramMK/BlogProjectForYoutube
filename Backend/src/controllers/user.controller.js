const { uploadFileToCloudinary, deleteFileToCloudinary } = require("../config/cloudinary.config");
    ...
    try{
      if (req.user.userId !== req.params.userId) {
        return next(
          errorHandler(
            403,
            "You Cant Update This Profile... Your are not valid user ",
          ),
        );
      }
      ...
      if(!ExistUser){
        return next(errorHandler(400 , "User DoesNot Exist ... SignUp First")) ; 
      }
      ...
      if (req.body.userName) {
        if (req.body.userName.length < 3 || req.body.userName.length > 16) {
          return next(
            errorHandler(400, "UserName Must Be between 3 to 16 characters"),
          );
        }
      ...
      if (req.file) {
        ...
        if(ExistUser.profilePicturePublicId){
            console.log("PUBLIC_ID ", ExistUser.profilePicturePublicId) ; 
            await deleteFileToCloudinary(ExistUser.profilePicturePublicId) ; 
        }

     
        const ProfilePictureUrl = await uploadFileToCloudinary(req.file);
        updateUser.profilePictrue = ProfilePictureUrl?.secure_url;
        updateUser.profilePicturePublicId = ProfilePictureUrl?.public_id ; 
      }
